...
## Compile and Install on Centos7 
#### Setup environment to install pgsql 

Export PostgreSQL version on an environment variable, we are installing version 10. If we need a different version to install we can install it just change the version number on the environment. All the other procedure can be same.  
...
Add PostgreSQL bin to PATH using environment, So all Postgres command will be available on the system.
...
#### Install PostgreSQL official YUM repo on System.
...
Compile & install pgaudit as per PostgreSQL version. To Compile the pgaudit plugin we need to install postgresql devel as per postgresql version, also we need gcc and openssl-devel. 
```
yum install -y postgresql${PG_VERSION?}-devel gcc openssl-devel
```
...
cd pgaudit
```
Checkout `REL_10_STABLE` branch (note that the stable branch may not exist for unreleased versions of PostgreSQL):
```
...
```
To apply the configration we need to rester the server, if we have already started before.
